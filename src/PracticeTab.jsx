...
function scoreColor(s)  {
  ...
  return "#22c55e";   // lime→green: CFFF04 is invisible on white, green works in both modes
}
function scoreBg(s) {
...
// ─── CSS variable aliases — work in both dark and light mode ─────────────────
// These are referenced as strings inside inline style objects.
// The app's CSS already defines :root (dark) and .light overrides for all of these.
const FG   = "var(--fg)";           // primary text
const FGM  = "var(--muted-fg)";     // muted / secondary text
const SURF = "var(--surface)";      // subtle card backgrounds
const BORD = "var(--card-border)";  // borders and bar tracks
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)", display: "block" }}>
        {/* Track ring — uses CSS var so it's visible in both modes */}
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
        ...
        <span style={{ fontSize:8, color:FGM, fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase", marginTop:2 }}>
          /100
        </span>
      </div>
    </div>
  ...
  return (
    <div style={{ marginBottom:12 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:5 }}>
        <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em", color:FGM }}>
          {label}
        </span>
        <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:14, fontWeight:700, color }}>
          {score}<span style={{ fontSize:9, color:FGM, marginLeft:1 }}>/10</span>
        </span>
      </div>
      {/* Track — BORD adapts: dark gray in dark mode, light gray in light mode */}
      <div style={{ height:5, borderRadius:99, background:BORD, overflow:"hidden", marginBottom:5 }}>
        <div style={{
      ...
      {note && <p style={{ fontSize:11, color:FGM, lineHeight:1.45, margin:0 }}>{note}</p>}
    </div>
  );
}
...
function ProgressCard({ parsedSwings, swingCount }) {
  ...
  return (
    <div style={{ background:SURF, border:`1px solid ${BORD}`, borderRadius:14, padding:"12px 14px" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
        <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:FGM, margin:0 }}>
          Your Progress
        </p>
        <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, color:FGM, background:SURF, borderRadius:99, padding:"2px 8px", border:`1px solid ${BORD}` }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom: (bestKey || latestFault) ? 8 : 0 }}>
        ...
        <div style={{ display:"flex", alignItems:"baseline", gap:5 }}>
          ...
          <span style={{ fontSize:10, color:FGM }}>pts since first swing</span>
        </div>
      </div>
      ...
      {bestKey && bestGain > 0 && (
        <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom: latestFault ? 6 : 0 }}>
          ...
          <p style={{ fontSize:11, color:FGM, margin:0, lineHeight:1.35 }}>
            <span style={{ color:FG, fontWeight:600 }}>Most improved: </span>
            {CAT_LABELS[bestKey]}
      ...
      {latestFault && (
        <div style={{ display:"flex", alignItems:"flex-start", gap:7 }}>
          ...
          <p style={{ fontSize:11, color:FGM, margin:0, lineHeight:1.4 }}>
            <span style={{ color:FG, fontWeight:600 }}>Fix next: </span>
...
function ComparisonTable({ swings }) {
  ...
  const cols = swings.slice(0, 3).reverse();

  return (
    <div style={{ background:SURF, border:`1px solid ${BORD}`, borderRadius:14, overflow:"hidden", marginBottom:16 }}>
      <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:FGM, padding:"12px 14px 8px", margin:0 }}>
        Swing Comparison
      </p>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
          <thead>
            <tr style={{ borderTop:`1px solid ${BORD}` }}>
              <th style={{ padding:"6px 14px", textAlign:"left", fontFamily:"'Space Grotesk',sans-serif", fontSize:8, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.06em", color:FGM, whiteSpace:"nowrap" }}>
                Category
              </th>
              {cols.map((s, i) => (
                <th key={i} style={{ padding:"6px 10px", textAlign:"center", fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, color: i === cols.length - 1 ? FG : FGM, whiteSpace:"nowrap" }}>
                  {i === cols.length - 1 ? "Latest" : i === 0 ? (cols.length > 2 ? "Oldest" : "Previous") : "Middle"}
                  <span style={{ display:"block", fontWeight:400, fontSize:8, color:FGM }}>{s.label}</span>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            <tr style={{ borderTop:`1px solid ${BORD}`, background:SURF }}>
              <td style={{ padding:"8px 14px", fontFamily:"'Space Grotesk',sans-serif", fontSize:10, fontWeight:700, textTransform:"uppercase", color:FGM }}>
                Overall
              </td>
              {cols.map((s, i) => {
                ...
                const col  = diff == null ? scoreColor(cur) : diff > 0 ? "#4ade80" : diff < 0 ? "#f87171" : FGM;
                return (
            ...
            {Object.entries(CAT_LABELS).map(([key, label], ri) => (
              <tr key={key} style={{ borderTop:`1px solid ${BORD}`, background: ri % 2 === 1 ? SURF : "transparent" }}>
                <td style={{ padding:"6px 14px", fontSize:11, color:FGM, whiteSpace:"nowrap" }}>{label}</td>
                {cols.map((s, i) => {
                  const cur  = s.parsed?.categories?.[key]?.score || 0;
                  const prev = i > 0 ? (cols[i-1].parsed?.categories?.[key]?.score || 0) : null;
                  const diff = prev != null ? cur - prev : null;
                  const col  = diff == null ? catColor(cur) : diff > 0 ? "#4ade80" : diff < 0 ? "#f87171" : FGM;
                  return (
    ...
    <div>
      ...
      <div style={{ padding:"14px 14px 8px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:16, padding:14, borderRadius:14, background:SURF, border:`1px solid ${color}33` }}>
          ...
          <div style={{ flex:1 }}>
            <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:FGM, margin:"0 0 4px" }}>
              Overall Score
            </p>
            <p style={{ fontSize:13, color:FGM, lineHeight:1.4, margin:"0 0 10px" }}>
        ...
        {/* ── Category bars ── */}
        <div style={{ background:SURF, border:`1px solid ${BORD}`, borderRadius:14, padding:"14px 14px 3px", marginBottom:14 }}>
          <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:FGM, margin:"0 0 12px" }}>
            Breakdown
          </p>
        ...
        {/* ── Primary fault ── */}
        {parsed.primaryFault && (
          <div style={{ border:"1px solid rgba(248,113,113,0.35)", background:"rgba(248,113,113,0.08)", borderRadius:14, padding:"12px 14px", marginBottom:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:7 }}>
              <span style={{ fontSize:15 }}>⚠️</span>
              <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:"#ef4444" }}>
                Fix This First
              </span>
            </div>
            <p style={{ fontSize:14, fontWeight:600, color:FG, lineHeight:1.45, margin:0 }}>
              {parsed.primaryFault}
            </p>
          </div>
        )}
        ...
        {/* ── Drill ── */}
        {parsed.drill && (
          <div style={{ background:SURF, border:`1px solid ${BORD}`, borderRadius:14, padding:"12px 14px", marginBottom:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:9 }}>
              <span style={{ fontSize:15 }}>🏌️</span>
              <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:FGM }}>
                Fix It Drill
              </span>
            </div>
            {typeof parsed.drill === "string" ? (
              <p style={{ fontSize:13, color:FG, lineHeight:1.6, margin:0 }}>{parsed.drill}</p>
            ) : (
              <ol style={{ margin:0, paddingLeft:0, listStyle:"none" }}>
                {parsed.drill.map((step, i) => (
                  <li key={i} style={{ display:"flex", gap:10, marginBottom:8 }}>
                    ...
                    <span style={{ fontSize:13, color:FG, lineHeight:1.55 }}>{step}</span>
                  </li>
                ))}
              </ol>
            )}
        ...
        {Array.isArray(parsed.positives) && parsed.positives.length > 0 && (
          <div style={{ marginBottom:12 }}>
            <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", color:FGM, margin:"0 0 8px" }}>
              What's Working
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {parsed.positives.map((pos, i) => (
                <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"9px 12px", borderRadius:10, background:"rgba(34,197,94,0.06)", border:"1px solid rgba(34,197,94,0.20)" }}>
                  <span style={{ color:"#22c55e", fontWeight:700, fontSize:13, lineHeight:"1.45", flexShrink:0 }}>✓</span>
                  <span style={{ fontSize:13, color:FG, lineHeight:1.45 }}>{pos}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── Coaching summary ── */}
        {parsed.summary && (
          <div style={{ background:SURF, border:`1px solid ${BORD}`, borderRadius:14, padding:"12px 14px", marginBottom:4 }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
              ...
              {speakText && (
                <button
                  ...
                  style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:9, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em", color: speaking ? "#22c55e" : FGM, background:"none", border:"none", cursor:"pointer", padding:"2px 0" }}
                >
                  {speaking ? "⏹ Stop" : "🔊 Read"}
                </button>
              )}
            </div>
            <p style={{ fontSize:13, color:FG, lineHeight:1.65, margin:0, fontStyle:"italic" }}>
              "{parsed.summary}"
            </p>
          </div>
        )}
